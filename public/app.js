...
function bind() {
  ...
  document.querySelectorAll('.tab').forEach(button => {
    button.addEventListener('click', async () => {
      ...
      if (button.dataset.view === 'newLotView') {
        await loadNextLotId();
      }
    });
  });
...
async function loadNextLotId() {
  const field = byId('newAutomaticId');
  ...
  try {
    const data = await adminRequest('obterProximoIdLote', {});
    field.value = data.proximoIdLote || 'Será gerado ao salvar';
  } catch (error) {
    field.value = 'Será gerado ao salvar';
  }
}
